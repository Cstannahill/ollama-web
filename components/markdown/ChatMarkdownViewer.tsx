...
export function ChatMarkdownViewer({ content, className = "" }: ChatMarkdownViewerProps) {
          ...
          if (line.startsWith('```')) {
            if (!inCodeBlock) {
              ...
              if (currentCode.trim()) {
                ...
                if (currentLang === 'javascript' || currentLang === 'js') label = 'JavaScript';
                if (currentLang === 'typescript' || currentLang === 'ts') label = 'TypeScript';
                if (currentLang === 'bash' || currentLang === 'sh') label = 'BASH';
              ...
              <ReactMarkdown
                ...
                components={{
                  ...
                  code({ inline, className, children, ...props }: {
                    ...
                    return (
                      <code className={`${className} bg-slate-800 px-1 py-0.5 rounded text-sm`} {...props}>
                        {children}
                      </code>
                    );
                  },
                  ...
                  h1({ children }) {
                    return <h1 className="text-xl font-bold text-emerald-400 mb-3">{children}</h1>;
                  },
                  h2({ children }) {
                    return <h2 className="text-lg font-semibold text-blue-400 mb-2 mt-4">{children}</h2>;
                  },
                  h3({ children }) {
                    return <h3 className="text-base font-medium text-purple-400 mb-2 mt-3">{children}</h3>;
                  },
                  ...
                  th({ children }) {
                    return <th className="bg-slate-800 border border-slate-700 px-3 py-2 text-left font-semibold text-xs">{children}</th>;
                  },
                  td({ children }) {
                    return <td className="border border-slate-700 px-3 py-2 text-xs">{children}</td>;
                  },
