...
import { Search, FileText, Download, Share2, Maximize2, Minimize2 } from "@/components/ui/icons";
...
<body>
  ...
  const parseSpecialSections = (markdown: string) => {
    ...
    const hasConfigBlocks = markdown.includes('## Basic Configuration') && 
    ...
    parts.forEach((part, index) => {
      ...
      const isConfigSection = title.toLowerCase().includes('config') && 
  ...
  return (
    <div className={`${isFullscreen ? 'fixed inset-0 z-[200] bg-slate-900' : ''} ${className}`}>
      <div className={`bg-slate-800 border-b border-slate-700 ${isFullscreen ? 'p-4' : 'px-4 py-3'}`}>
        {/* Header */}
      ...
      {/* Content */}
      <div className={`${isFullscreen ? 'flex-1 overflow-auto p-6' : 'p-6'} space-y-6`}>
        {sections.map((section, index) => {
          if (section.type === 'config' && section.tabs && section.tabs.length > 0) {
            return (
                  ...
                  components={{
                    ...
                    code({ node, inline, className, children, ...props }: any) {
                      ...
                      if (!inline && language) {
                        ...
                        if (language === "mermaid") {
                          return <MermaidComponent chart={code} className="my-4" />;
                        }
                    ...
                    h1({ children }) {
                      return <h1 className="text-3xl font-bold text-emerald-400 mb-4">{children}</h1>;
                    },
                    h2({ children }) {
                      return <h2 className="text-2xl font-semibold text-blue-400 mb-3 mt-8">{children}</h2>;
                    },
                    h3({ children }) {
                      return <h3 className="text-xl font-medium text-purple-400 mb-2 mt-6">{children}</h3>;
                    },
                    ...
                    th({ children }) {
                      return <th className="bg-slate-800 border border-slate-700 px-4 py-2 text-left font-semibold">{children}</th>;
                    },
                    td({ children }) {
                      return <td className="border border-slate-700 px-4 py-2">{children}</td>;
                    },
                  }}
                >
                  {section.content}
